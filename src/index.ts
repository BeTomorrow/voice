...
class RCTVoice {
	...
	private _voiceEmitter: NativeEventEmitter | null = Platform.OS !== "web" ? new NativeEventEmitter(this.Voice) : null;
	...
	getSpeechRecognitionServices() {
		if (Platform.OS !== "android") {
			invariant(this.Voice, "Speech recognition services can be queried for only on Android");
			return;
		}
	...
	isRecognizing(): Promise<boolean> {
		return new Promise((resolve) => {
			this.Voice.isRecognizing((isRecognizing: boolean) => resolve(isRecognizing));
		});
	}

	onSpeechStart(fn: (e: SpeechStartEvent) => void): EmitterSubscription {
		return this.addListenerToEvent(VoiceEventType.onSpeechStart, (e: SpeechStartEvent) => {
	...
	onSpeechRecognized(fn: (e: SpeechRecognizedEvent) => void): EmitterSubscription {
		return this.addListenerToEvent(VoiceEventType.onSpeechRecognized, (e) => fn(e));
	}

	onSpeechEnd(fn: (e: SpeechEndEvent) => void): EmitterSubscription {
		return this.addListenerToEvent(VoiceEventType.onSpeechEnd, (e: SpeechEndEvent) => {
			this.clearAllTimeout();
			fn(e);
		});
	}
	onSpeechError(fn: (e: SpeechErrorEvent) => void): EmitterSubscription {
		return this.addListenerToEvent(VoiceEventType.onSpeechError, (e: SpeechErrorEvent) => {
			this.clearAllTimeout();
			fn(e);
		});
	}
	onSpeechResults(fn: (e: SpeechResultsEvent) => void): EmitterSubscription {
		return this.addListenerToEvent(VoiceEventType.onSpeechResults, (e: SpeechResultsEvent) => {
	...
	onSpeechPartialResults(fn: (e: SpeechResultsEvent) => void): EmitterSubscription {
		return this.addListenerToEvent(VoiceEventType.onSpeechPartialResults, (e: SpeechResultsEvent) => {
	...
	onSpeechVolumeChanged(fn: (e: SpeechVolumeChangeEvent) => void) {
		return this.addListenerToEvent(VoiceEventType.onSpeechVolumeChanged, (e) => fn(e));
	}
